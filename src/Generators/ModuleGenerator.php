...
namespace MdTech\Modules\Generators;
...
use MdTech\Modules\Support\Config\ControllerConfigReader;
...
class ModuleGenerator extends Generator
{
    ...
    /**
     * if is the child of thd modules.
     *
     * @var bool
     */
    protected $child = '';
...
//        }
    ...
    public function generateFolders()
    {
        foreach ($this->getFolders() as $key => $folder) {
            ...
            try{
                $path = $this->module->getAbsentFilePath($folder->getPath(), $this->getName());
...
//        }
        ...
        foreach ($this->module->config('paths.part') as $part => $value){
            $partConfig = PartConfigReader::read($part);
            if($partConfig->generate() == false) continue;

            if(!$partConfig->onlyService()){
            ...
            if (GenerateConfigReader::read('service')->generate() === true) {
                $this->console->call('module:make-service', [
                        'service' => $this->getName() . $this->getChild(),
                        'module' => $this->getName(),
                        'part' => $part
                    ] + $options);
            }
        ...
        if (GenerateConfigReader::read('model')->generate() === true) {
            $this->console->call('module:make-model', [
                    'model' => $this->getName() . $this->getChild(),
                    'module' => $this->getName(),
                ]+$options);
        }
    }
