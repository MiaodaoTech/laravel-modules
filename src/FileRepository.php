...
use MdTech\Modules\Exceptions\FileNotFoundException;
use MdTech\Modules\Exceptions\InvalidAssetPath;
use MdTech\Modules\Exceptions\ModuleNotFoundException;
use MdTech\Modules\Process\Installer;
use MdTech\Modules\Process\Updater;
...
abstract class FileRepository implements RepositoryInterface, Countable
{
    ...
    /**
     * @name getFilePath
     * @description 获取文件路径或直接返回false
     * @param $path
     * @param $module
     * @return string|bool
     */
    public function getFilePath($path, $module){
        $path = $this->getPath() . '/' . $path . '/' . $module . '/';
        if(!$this->getFiles()->isDirectory($path)){
            throw new FileNotFoundException("File [{$module}] does not exist!");
        }
        return $path;
    }

    /**
     * @name getAbsentFilePath
     * @description 获取不存在的文件路径或直接返回false
     * @param $path
     * @param $module
     * @return string|bool
     * @author h2odumpling
     * @date 2023/7/12
     */
    public function getAbsentFilePath($path, $module){
        $path = $this->getPath() . '/' . $path . '/' . $module . '/';
        if($this->getFiles()->isDirectory($path)){
            throw new FileAlreadyExistException("File [{$module}] already exist!");
        }
        return $path;
    }
     ...
     * @return string
     * @throws \MdTech\Modules\Exceptions\ModuleNotFoundException
     */
